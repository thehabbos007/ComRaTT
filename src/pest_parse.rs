...
impl Prog {
    pub fn parse_pest(input: &str) -> Result<Prog, String> {
        let pairs = match ComRaTTParser::parse(Rule::program, input) {
            Ok(pairs) => pairs,
            Err(error) => {
                // let span = Range::new(error.location.pos(), error.location.pos());
                // return Err(ComRaTTError::from_span(error.to_string(), span, input));
            ...
            match pair.as_rule() {
                Rule::function_def => {
                    ...
                    while next.as_rule() == Rule::identifier {
                        ...
                        next = pairs
                            .next()
                            .unwrap_or_else(|| panic!("Expected expression after arguments"));
                    }
...
fn parse_expression_atom(pair: Pair<Rule>) -> Expr {
    match pair.as_rule() {
        ...
        Rule::fun_expr => {
            ...
            // The body expression
            let body = parse_expression(pairs);
        ...
        Rule::delay_expr => {
            ...
            let channel_set_pair = delay_inner.next().unwrap();
            let mut channel_set = HashSet::new();

            for channel in channel_set_pair.into_inner() {
        ...
        //     dbg!(&app_expr);
        //     let exp = parse_expression(app_expr.next().unwrap().into_inner());
        //     dbg!(&exp);

        //     if let Some(exp_arg) = app_expr.next() {
        //         let arg = parse_expression(exp_arg.into_inner());
        //         return Expr::App(Box::new(exp), Box::new(arg));
        //     }
